...
• Canʼt call malloc() to allocate storage for this info
• Canʼt call free() to deallocate the storage, either

Solution:
• Store the information directly in the block
  这个非常关键！！！ 直接存在block itself里面！！！！所以其实不是linked list node那种
  也很合理 因为我们就是要实现malloc怎么能用malloc呢
  所以做法就是每次多allocate一个header的memory用来作为header 存block的信息！！
  fixed size的不需要因为他不需要存size只是一个指针就好
• Since the memory isnʼt being used for anything anyway
...
• Keep list in order of increasing addresses
  这里的free list是sorted by memory address – pointers with lower addresses first
    • Makes it easier to coalesce adjacent free blocks 这也是容易merge的好处！！！
*/
 ...
 */
// 这个struct的定义很关键 而且还利用了union的特性来保证alignment
union header 
{
  struct 
  {
...
static Header* freep = nullptr;  // Free list starting point
    ...
    for (; ; prevp = currp, currp = currp->s.next) 
    {
        ...
        if (currp->s.size >= nunits) 
        {
            ...
            */
            ...
            {
                // 这里是把当前的size shrink了 然后把后面的node作为满足nunits的node
        ...
        if (currp == freep) 
        {
            // 这里是就是call system call to allocate new block!!!
...
// 这个morecore就是call into kernel to request more memory!!!!!
...
void kandr_free(void* ptr) {
    ...
    Header *insertp;
    Header *currp;
    // 这里的header node 是 sorted by address的！！！！！
    ...
    */
    // 我知道了这里最后满足的条件就是 (currp < insertp) && (insertp < currp->s.next) 
    // 所以for loop 只要!(final condition) 就可以啦！！！
